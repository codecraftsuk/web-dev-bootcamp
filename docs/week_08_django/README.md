...
**Note for studnets:** In the code block above, replace the inner `{}` with `%%` for the `csrf_token` like in previous examples e.g.:`{% for %}`. The brackets have been changed to avoid conflicts in this documentation.
